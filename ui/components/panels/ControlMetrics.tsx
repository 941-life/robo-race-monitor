...
function MetricCard({ label, value, unit = "", precision = 2, warning, danger }: MetricCardProps) {
  ...
  const valueColor = danger ? "text-[#e22718]" : warning ? "text-[#f4b400]" : "text-white";
  const accentBar = danger
    ? "border-l-[#e22718]"
    : warning
      ? "border-l-[#f4b400]"
  ...
  return (
    <div className={`bg-[#1a2129] border border-l-2 border-white/10 p-2.5 flex flex-col gap-1.5 ${accentBar}`}>
      <span className="text-xs font-bold uppercase tracking-[1.5px] text-[#bbbbbb] leading-none">
        {label}
      </span>
      <div className="flex items-baseline gap-1.5">
        <span className={`text-xl font-bold tabular-nums leading-none font-mono ${valueColor}`}>
          {display}
        </span>
    ...
    <div className="bg-[#262e38] border border-white/10 flex flex-col overflow-hidden shrink-0">
      ...
      {!hasData ? (
        <div className="flex flex-col items-center justify-center p-8 gap-3">
          ...
          <span className="text-sm text-[#6b6b6b]">Waiting for control data</span>
        </div>
      ) : (
        <div className="grid grid-cols-2 gap-px bg-white/5">
          <MetricCard
            ...
            value={cte}
            precision={3}
            warning={Math.abs(cte ?? 0) > 0.3}
            danger={Math.abs(cte ?? 0) > 0.6}
          />
          <MetricCard label="Steer" value={steer} precision={1} />
